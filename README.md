...
排印独立于主题：作用到全部文字或仅代码、字号 11–18 px、7 种代码字体带实时预览。`system` 一键回到 DSH 原生外观，排印设置保留。面板跟着 DSH 界面语言走（中文 / English），切换即时跟随。
...
| **中英同字体字体档** | 新增 `Maple Mono NF CN`：中英文同一款字体、中英宽度 2:1，代码块与表格里的中英混排不再错位。本机安装后即可选用，未装则灰显标注「本机未装」 | 无 |
