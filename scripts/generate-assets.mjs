...
// ── palette-matrix ──（34 个微缩 DSH 界面预览：每卡片=真实主题渲染的迷你界面）
function miniUI(x, y, w, h, c) {
  const bg = c.background || '#161616'
  const panel = c.background || '#1c1c1c'
  ...
  return '<g>' +
    ...
    // 顶栏
    '<rect x="' + x + '" y="' + y + '" width="' + w + '" height="16" rx="8" fill="' + panel + '"/>' +
    '<circle cx="' + (x + 10) + '" cy="' + (y + 8) + '" r="2.5" fill="' + primary + '"/>' +
    '<rect x="' + (x + 18) + '" y="' + (y + 6.5) + '" width="42" height="2.5" rx="1.25" fill="' + muted + '" opacity="0.5"/>' +
    // 消息气泡
    '<rect x="' + (x + 8) + '" y="' + (y + 21) + '" width="114" height="13" rx="3.5" fill="' + panel + '" stroke="' + border + '"/>' +
    '<rect x="' + (x + 12) + '" y="' + (y + 25.5) + '" width="42" height="2.5" rx="1.25" fill="' + muted + '" opacity="0.6"/>' +
    '<rect x="' + (x + w - 8 - 58) + '" y="' + (y + 38) + '" width="58" height="11" rx="3.5" fill="' + primary + '" opacity="0.85"/>' +
    // 代码块（语法高亮）
    '<rect x="' + (x + 8) + '" y="' + (y + 53) + '" width="' + (w - 16) + '" height="25" rx="4" fill="' + panel + '" stroke="' + border + '"/>' +
    '<rect x="' + (x + 12) + '" y="' + (y + 59) + '" width="30" height="3" rx="1.5" fill="' + keyword + '"/>' +
    '<rect x="' + (x + 45) + '" y="' + (y + 59) + '" width="18" height="3" rx="1.5" fill="' + string + '"/>' +
    '<rect x="' + (x + 12) + '" y="' + (y + 67) + '" width="52" height="3" rx="1.5" fill="' + muted + '" opacity="0.35"/>' +
    // 输入框
    '<rect x="' + (x + 8) + '" y="' + (y + 82) + '" width="' + (w - 16) + '" height="12" rx="5" fill="' + panel + '" stroke="' + border + '"/>' +
    '<circle cx="' + (x + w - 20) + '" cy="' + (y + 88) + '" r="3" fill="' + primary + '"/>' +
    '</g>'
}
function matrixDoc(lang) {
  const T = L[lang]
  const titleLine = lang === 'zh' ? '款官方主题 · 全部官方配色' : 'official opencode themes'
  const COLS = 6, CW = 196, UIH = 100, GAP = 10, PAD_X = 20, PAD_Y = 96
  const rows = Math.ceil(names.length / COLS)
  const W = PAD_X * 2 + COLS * CW + (COLS - 1) * GAP
  const H = PAD_Y + rows * (UIH + 20) + (rows - 1) * GAP + 20
  const cards = names.map((n, i) => {
    const c = data[n]
    const col = i % COLS, row = Math.floor(i / COLS)
    const x = PAD_X + col * (CW + GAP), y = PAD_Y + row * ((UIH + 20) + GAP)
    const focus = n === 'opencode' ? '<rect x="' + (x - 1.5) + '" y="' + (y - 1.5) + '" width="' + (CW + 3) + '" height="' + (UIH + 3) + '" rx="10" fill="none" stroke="#FAB283" stroke-opacity="0.85" stroke-width="1.5"/>' : ''
    return '<g>' +
      focus +
      miniUI(x, y, CW, UIH, c) +
      '<text x="' + (x + 2) + '" y="' + (y + UIH + 15) + '" font-family="' + MONO + '" font-size="10.5" letter-spacing="0.5" fill="#9a9aa2">' + esc(n) + '</text>' +
      '</g>'
  }).join('')
  return [
    '<svg xmlns="http://www.w3.org/2000/svg" width="' + W + '" height="' + H + '" viewBox="0 0 ' + W + ' ' + H + '">',
    '<defs><linearGradient id="bigNum" x1="0" y1="0" x2="1" y2="0"><stop offset="0%" stop-color="#FAB283"/><stop offset="55%" stop-color="#9D7CD8"/><stop offset="100%" stop-color="#56B6C2"/></linearGradient></defs>',
    '<rect width="100%" height="100%" fill="#0d0d0d"/>',
    '<text x="' + PAD_X + '" y="82" font-family="' + SANS + '" font-size="64" font-weight="800" fill="url(#bigNum)">34</text>',
    '<text x="' + (PAD_X + 130) + '" y="52" font-family="' + SANS + '" font-size="16" font-weight="600" fill="#e8e8e8">' + esc(titleLine) + '</text>',
    '<text x="' + (PAD_X + 130) + '" y="74" font-family="' + SANS + '" font-size="12.5" fill="#8b8b95">' + esc(T.matrixSub) + '</text>',
    cards,
    '</svg>',
  ].join('\n')
}
